...
router.put('/:wordId', auth, function(req, res) {
  if (!Number.isInteger(req.body.index) || !Number.isInteger(req.body.score)) {
    return res.status(400).json({
      code: 'missingValues',
      message: 'Required index and score to be integers'
    })
  } else if (req.body.index < 0 || req.body.index > 5) {
    return res
      .status(400)
      .json({ code: 'invalidIndex', message: 'Index must be between 0 and 5' })
  } else if (req.body.score < 0 || req.body.score > 5) {
    return res
      .status(400)
      .json({ code: 'invalidScore', message: 'Score must be between 0 and 5' })
  }

  Score.findOne({ userId: req.user._id, wordId: req.params.wordId }, function(
    err,
    score
  ) {
    if (err) {
